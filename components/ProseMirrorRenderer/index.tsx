...
import type { PMDoc, PMNode, PMMark } from '~/lib/ProseMirrorTypes';
...
function renderBlock(node: PMNode, key: number): React.ReactNode {
  ...
  switch (node.type) {
    ...
    case 'heading': {
      ...
      const classes =
        level === 1 ? 'mb-6 mt-12 text-3xl font-bold' : level === 2 ? 'mb-4 mt-8 text-2xl font-semibold' : 'mb-4 mt-6 text-xl font-semibold';
      return (
    ...
    case 'blockquote':
      return (
        <blockquote key={key} className="mb-6 border-l-4 border-primary pl-4 italic">
    ...
    case 'image': {
      ...
      return (
        <img key={key} src={src} alt={alt} className="my-4 max-w-full rounded-lg" />
      );
    }
    case 'figure':
      return (
    ...
    case 'figcaption':
      return (
        <figcaption key={key} className="mt-2 text-center text-sm text-muted-foreground">
          {renderInlineNodes(content)}
        </figcaption>
      );
    case 'table':
